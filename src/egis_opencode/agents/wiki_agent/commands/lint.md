---
description: 知识库全局健康度检查。扫描 wiki/ 目录，检测死链（引用不存在的双链）、孤儿页面（无任何页面引用）、未同步索引（文件存在但未在 index.md 注册）和知识冲突，输出结构化体检报告。
---
...
将软件工程中的"静态代码分析"引入知识管理。定期运行此命令，找出知识库长期演进中产生的：死链、孤岛、未同步索引、认知冲突。
