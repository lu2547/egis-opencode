---
description: 将 raw/ 目录下的原始资料编译到 wiki/ 中。/ingest 扫描 raw/ 下所有未归档文件；/ingest <path> 处理指定文件。处理完成后源文件自动移动到 raw/09-archive/ 归档。绝对忽略 raw/09-archive/ 目录。
---
...
2. **用户执行 `/ingest <path>`**：仅处理指定文件（见文末用户参数）。
